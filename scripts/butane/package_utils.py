...
class Behavior(object):
    ...
    def __init__(self, package, bdict, lang):
        ...
        data.
        :param package: instance of the package class that contains the
                        behavior.
        ...
        self.launch_path = os.path.join(os.path.split(self.package.path)[1],
                                        self.rel_path)
        self.utterable_name = self.get_utterable_name()
    ...
    def get_utterable_name(self):
        """Return a name you can speak."""
        if self.rel_path == '.':
            return self.package.utterable_name
        elif not self.name:
            return make_utterable('{}-{}'.format(self.package.name,
                                                 self.rel_path.split('/')[-1]))
        else:
            return make_utterable('{}-{}'.format(self.package.name, self.name))

    def is_triggerable(self):
        """Check if a behavior is triggerable by a user."""
        has_ts = bool(self.trigger_sentences)
        is_interactive = self.nature == 'interactive'
        return has_ts and is_interactive
...
def get_packages(lang, session=None):
...
def make_utterable(name):
    ...
    def utterable(name):
        ...
        # print replaced_camelcase
        replaced_separators = re.sub(r'[\./_-]', ' ', replaced_camelcase)
        # print replaced_separators
...
def main():
    ...
    ses = qi.Session()
    ses.connect('raphael.local')

    # packages = [PACKAGE_1, PACKAGE_2]
    for pack in get_packages('en_US', ses):
        pprint(vars(pack))
        print
